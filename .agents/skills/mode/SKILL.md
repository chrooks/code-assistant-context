...
Switch your Codex Code interaction mode for this session (or persistently).
...
Use the Bash tool to write the mode name (uppercase) to `~/.codex/learning-mode`:

```bash
echo "SOCRATIC" > ~/.codex/learning-mode   # or ANNOTATOR / STANDARD
```
...
```
[STANDARD MODE] Switched to default Codex Code behavior. No special learning mode active.
```

No behavioral rules are applied. This is the default Codex Code experience.
